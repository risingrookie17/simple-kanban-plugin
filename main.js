"use strict";var w=Object.defineProperty;var E=Object.getOwnPropertyDescriptor;var L=Object.getOwnPropertyNames;var _=Object.prototype.hasOwnProperty;var D=(i,n)=>{for(var e in n)w(i,e,{get:n[e],enumerable:!0})},z=(i,n,e,t)=>{if(n&&typeof n=="object"||typeof n=="function")for(let a of L(n))!_.call(i,a)&&a!==e&&w(i,a,{get:()=>n[a],enumerable:!(t=E(n,a))||t.enumerable});return i};var B=i=>z(w({},"__esModule",{value:!0}),i);var I={};D(I,{default:()=>y});module.exports=B(I);var u=require("obsidian");var k=require("obsidian");var j={todo:"\u5F85\u5904\u7406",doing:"\u8FDB\u884C\u4E2D",done:"\u5DF2\u5B8C\u6210"},S=["todo","doing","done"],T={\u5F00\u53D1:"\u{1F4BB} \u5F00\u53D1",\u8BBE\u8BA1:"\u{1F3A8} \u8BBE\u8BA1",\u6587\u6863:"\u{1F4DD} \u6587\u6863",\u6D4B\u8BD5:"\u{1F9EA} \u6D4B\u8BD5",\u8FD0\u7EF4:"\u2699\uFE0F \u8FD0\u7EF4",\u5176\u4ED6:"\u{1F4E6} \u5176\u4ED6"},h={high:{label:"\u9AD8",color:"#ef4444"},medium:{label:"\u4E2D",color:"#f59e0b"},low:{label:"\u4F4E",color:"#22c55e"}};var m=require("obsidian");function C(i){let n=i.split(`
`),e=-1;for(let a=0;a<Math.min(n.length,20);a++)if(n[a].trim()==="---"){e=a;break}if(e===-1)return null;let t=-1;for(let a=e+1;a<Math.min(n.length,50);a++)if(n[a].trim()==="---"){t=a;break}return t===-1?null:{start:e,end:t,contentStart:e+1,contentEnd:t}}function M(i){let n=C(i);if(!n)return{};let t=i.split(`
`).slice(n.contentStart,n.contentEnd),a={};for(let r of t){let s=r.trim();if(!s||s.startsWith("#"))continue;let o=s.indexOf(":");if(o===-1)continue;let p=s.substring(0,o).trim(),l=s.substring(o+1).trim();a[p]=l}return a}function F(i,n){let e=n||new Date().toISOString().split("T")[0],t=["---"];i.status&&(t.push(`  status: ${i.status}`),i.status==="doing"&&!i.start_date&&t.push(`  start_date: ${e}`),i.status==="done"&&!i.completed_date&&t.push(`  completed_date: ${e}`));let a=["status"];for(let[r,s]of Object.entries(i))a.includes(r)||s!=null&&s!==""&&t.push(`  ${r}: ${s}`);return t.push("---"),t.join(`
`)}function $(i){let n=i.split(`
...
`;try{await this.app.vault.create(o,l);let d=this.app.vault.getAbstractFileByPath(o);if(d&&d instanceof m.TFile)return await this.loadTaskFromFile(d)}catch(d){console.error("Failed to create task:",d)}return null}async deleteTask(n){let e=this.app.vault.getAbstractFileByPath(n.filePath);if(!e||!(e instanceof m.TFile))return!1;try{return await this.app.vault.delete(e),!0}catch(t){return console.error("Failed to delete task:",t),!1}}};var v=class extends k.View{constructor(e,t){super(e);this.board=null;this.draggedTask=null;this.storage=new x(t.app)}getViewType(){return"simple-kanban-view"}getDisplayText(){return"Simple Kanban"}async onOpen(){await this.loadBoard(),this.render(),this.registerFileWatcher()}async onClose(){}async loadBoard(){let e=await this.storage.scanProjects();if(e.length===0){this.board={projects:[],selectedProject:"",columns:[],lastScanTime:Date.now()};return}let t=e[0].id;this.board=await this.storage.buildKanbanBoard(t)}async switchProject(e){this.board&&(this.board=await this.storage.buildKanbanBoard(e),this.render())}registerFileWatcher(){this.app.vault.on("modify",async e=>{e instanceof k.TFile&&e.name.startsWith("T-")&&await this.refresh()}),this.app.vault.on("create",async e=>{e instanceof k.TFile&&e.name.startsWith("T-")&&await this.refresh()}),this.app.vault.on("delete",async e=>{e instanceof k.TFile&&e.name.startsWith("T-")&&await this.refresh()})}async refresh(){if(console.log("Refreshing kanban board..."),!this.board){console.log("No board to refresh");return}console.log("Current project:",this.board.selectedProject),this.board=await this.storage.buildKanbanBoard(this.board.selectedProject),console.log("Board tasks:",this.board.columns.map(e=>`${e.name}: ${e.tasks.length}`)),this.render(),console.log("Render complete")}render(){if(!this.board)return;let e=this.containerEl;if(e.empty(),e.addClass("simple-kanban"),this.renderHeader(e),this.board.projects.length===0){this.renderEmptyState(e);return}this.renderColumns(e)}renderHeader(e){let t=e.createDiv("simple-kanban-header"),a=t.createDiv("simple-kanban-header-controls"),r=a.createDiv("simple-kanban-project-wrapper"),s=r.createEl("select");s.className="simple-kanban-project-select";let o=r.createSpan("simple-kanban-project-arrow");o.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div class="simple-kanban-modal-field">
        ...
        <select id="task-priority">
          ${t.map(r=>`<option value="${r}" ${r==="medium"?"selected":""}>${h[r].label}</option>`).join("")}
        </select>
      </div>
    ...
    `,document.body.appendChild(r);let s=()=>{document.body.removeChild(r)};return r.querySelector(".simple-kanban-modal-backdrop")?.addEventListener("click",s),r.querySelector(".simple-kanban-modal-close")?.addEventListener("click",s),r.querySelector(".simple-kanban-modal-close")?.addEventListener("click",s),r.querySelector(".simple-kanban-modal-save")?.addEventListener("click",async()=>{await a(),s()}),{open:()=>{r.querySelector("#task-title")?.focus()}}}};var y=class extends u.Plugin{async onload(){console.log("Loading Simple Kanban plugin"),this.loadStyles(),this.registerView("simple-kanban-view",n=>new v(n,this)),this.addCommand({id:"open-kanban",name:"\u6253\u5F00\u770B\u677F",callback:()=>this.openKanban()}),this.addRibbonIcon("layout","\u6253\u5F00\u770B\u677F",()=>{this.openKanban()}),this.addSettingTab(new P(this))}loadStyles(){this.app.workspace.onLayoutReady(()=>{let n="simple-kanban-styles-v2";if(!document.getElementById(n)){let e=document.createElement("style");e.id=n,e.textContent=`
